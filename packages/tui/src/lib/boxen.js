...
const boxContent = (content, contentWidth, options) => {
	...
	const top = colorizeBorder(
		NEWLINE.repeat(options.margin.top) +
			marginLeft +
			chars.topLeft +
			(options.title
	...
	const bottom = colorizeBorder(
		marginLeft +
			...
			chars.bottomRight +
			NEWLINE.repeat(options.margin.bottom)
	);
...
const determineDimensions = (text, options) => {
	...
	const widest =
		widestLine(wrapAnsi(text, columns - BORDERS_WIDTH, { hard: true, trim: false })) +
	...
	if (!widthOverride) {
		...
		options.width = Math.min(
			options.width,
			columns - BORDERS_WIDTH - options.margin.left - options.margin.right
		);
	}
