...
const exitHelpers = {
	exitProcess: ({ code = 1, message }, cache) => {
		exitHelpers.logExit(code, { message }, cache);
		process.exit(code);
	},

	logExit: (code = 0, error, cache) => {
		...
		if (code === 1) {
			...
			if (cache && cache.isSetupDone) {
				...
				console.log(
					chalk.yellowBright.bgBlack("Press [Ctrl]+[C] to force exit")
				);
			}
		}
	},
	...
	handleExit: (cache) => {
		try {
			...
			try {
				...
				console.log(
					chalk.black.bgGreenBright(
						`		> Trade history saved to ${chalk.bold(
							"./temp/tradeHistory.json"
						)} `
					)
				);
